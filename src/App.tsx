...
import { 
  ...
  downloadCompleteCaseReportPDF,
  downloadStage11PDF,
  downloadStage12PDF,
  downloadClientActionPDF
} from "./lib/pdfExport";
...
import { 
  ...
  Smartphone, Calculator, Hourglass, ShieldAlert, BookOpen, Target
} from "lucide-react";
import {
  signInWithGoogle,
  logoutUser,
...
export default function App() {
  ...
  const handleGlobalPDFDownload = async (mode: 'complete' | 'stage11' | 'stage12' | 'brief' | 'summary' | 'draft' = 'complete') => {
    ...
    try {
      ...
      if (mode === 'stage11') {
        await downloadStage11PDF(selectedCase);
        return;
      }
      if (mode === 'stage12') {
        await downloadStage12PDF(selectedCase);
        return;
      }
      if (mode === 'brief') {
        await downloadClientActionPDF(selectedCase);
        return;
      }
...
${selectedCase.stage6?.available?.map(d => `✓ ${d}`).join("\n") || "N/A"}
      ...
      let reportType = mode === "draft" ? "AI LEGAL DRAFT" : "CASE EXECUTIVE SUMMARY";
      let docType = selectedCase.stage1?.category || "Case Executive Brief";
                  ...
                  <div className="relative inline-flex rounded-xl shadow-3xs">
                    ...
                    {showPdfMenu && (
                      <div className="absolute right-0 top-full mt-1 w-64 bg-white rounded-xl shadow-xl border border-slate-200 z-50 p-1.5 text-xs text-slate-700">
                        <div className="px-2 py-1.5 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider border-b border-slate-100 mb-1">
                          {t("அறிக்கை வடிவங்கள் (PDF Formats)", "PDF Export Formats")}
                        </div>
                        <button
                          onClick={() => handleGlobalPDFDownload('complete')}
                          className="w-full text-left px-2.5 py-2 hover:bg-purple-50 hover:text-purple-900 rounded-lg flex items-center gap-2 font-bold cursor-pointer transition-colors"
                        >
                          <FileText className="h-4 w-4 text-purple-700 shrink-0" />
                          <div>
                        ...
                        <button
                          onClick={() => handleGlobalPDFDownload('stage11')}
                          className="w-full text-left px-2.5 py-2 hover:bg-purple-50 hover:text-purple-900 rounded-lg flex items-center gap-2 font-bold cursor-pointer transition-colors"
                        >
                          <BookOpen className="h-4 w-4 text-purple-700 shrink-0" />
                          <div>
                            <div>{t("நிலை 11 - முன்மாதிரி தீர்ப்புகள்", "Stage 11 — Precedent Intelligence")}</div>
                            <div className="text-[10px] font-normal text-slate-500">{t("உயர் நீதிமன்றத் தீர்ப்புகள் & அரசாணைகள்", "High Court Precedents & GOs")}</div>
                          </div>
                        </button>
                        <button
                          onClick={() => handleGlobalPDFDownload('stage12')}
                          className="w-full text-left px-2.5 py-2 hover:bg-purple-50 hover:text-purple-900 rounded-lg flex items-center gap-2 font-bold cursor-pointer transition-colors"
                        >
                          <Target className="h-4 w-4 text-purple-700 shrink-0" />
                          <div>
                            <div>{t("நிலை 12 - சட்ட உத்தி சிமுலேட்டர்", "Stage 12 — Strategy Simulator")}</div>
                            <div className="text-[10px] font-normal text-slate-500">{t("சட்ட வழிமுறை, இடைவெளிகள் & பதில் வாதங்கள்", "Legal Route, Gaps & Counterarguments")}</div>
                          </div>
                        </button>
                        <button
                          onClick={() => handleGlobalPDFDownload('brief')}
                          className="w-full text-left px-2.5 py-2 hover:bg-purple-50 hover:text-purple-900 rounded-lg flex items-center gap-2 font-bold cursor-pointer transition-colors border-t border-slate-100 mt-1 pt-1.5"
                        >
                          <User className="h-4 w-4 text-amber-600 shrink-0" />
                          <div>
                            <div>{t("வாடிக்கையாளர் நடவடிக்கை PDF", "Client Action Plan PDF")}</div>
                            <div className="text-[10px] font-normal text-slate-500">{t("செயல் திட்டம், ஆவணப் பட்டியல் & கால அளவு", "Action Plan, Document Checklist & Brief")}</div>
                          </div>
                        </button>
                        <button
