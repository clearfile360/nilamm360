...
export const isSupabaseConfigured = Boolean(supabaseUrl) && Boolean(supabaseAnonKey) && !supabaseUrl.includes("placeholder");
export const isSupabaseMockEnabled = !isSupabaseConfigured;

// Provide safe fallback configuration if environment variables are not yet provided
const effectiveUrl = supabaseUrl || "https://placeholder-domain.supabase.co";
...
export const syncCaseToCloud = async (userId: string, caseData: any) => {
  ...
  try {
    ...
    if (error) {
      console.warn("Supabase case sync error:", error);
    }
  } catch (err) {
...
export const fetchCloudCases = async (userId: string) => {
  ...
  try {
    ...
    console.warn("Fetch cloud cases exception:", err);
    return [];
  }
};
